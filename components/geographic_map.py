...
def create_state_map(df: pd.DataFrame, metric: str = 'yield_rate') -> go.Figure:
    ...
    avg_value = state_data[metric].mean()
    
    if metric == 'yield_rate':
        insight = f"Highest yield: {top_state['state']} ({top_state['yield_rate']:.1f}%) | National avg: {avg_value:.1f}%"
        ...
        insight = f"Highest admit rate: {top_state['state']} ({top_state['admit_rate']:.1f}%) | National avg: {avg_value:.1f}%"
    else:
